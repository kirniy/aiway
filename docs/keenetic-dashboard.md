...
Это те случаи, о которых стоит помнить заранее.

### 1. На роутере активен AWG и он владеет default route

Самая неприятная ловушка на Keenetic: DNS провайдера может начать ходить через AWG-туннель.

Именно поэтому `AIWAY Manager` при `AIWAY OFF`:

- очищает DoT upstream
- оставляет только DNS провайдера
- закрепляет провайдерские DNS за `ISP/WAN`

Иначе кажется, будто `AIWAY OFF` «ломает интернет», хотя на самом деле ломается путь до DNS провайдера.

### 2. Дашборд не должен врать про DNS

Панель показывает не только желаемое состояние, но и реальный runtime state роутера.

Это важно, потому что на Keenetic:

- конфиг уже может быть сохранён
- а `ndnproxy` ещё жить в старом состоянии

Поэтому в коде используется проверка фактического runtime-конфига, а не только полей `desiredDnsOn`.

### 3. VPS уже настроен вручную

Если у вас старая рабочая схема на VPS, не надо сразу переводить её в полностью управляемый режим.

Используйте `Legacy VPS`:

- панель читает состояние
- домены правятся точечно
- destructive-операции скрыты

Это снижает шанс сломать уже работающий сервер.

### 4. Роутер после перезагрузки

После reboot нужно проверять две вещи:

- что `AIWAY Manager` поднялся
- что runtime DNS состояние совпадает с сохранённым

Особенно если на роутере активны AWG-туннели и сложная маршрутизация.

### 5. SSH есть, но панель не должна ломать сервер

Даже если SSH работает, это не значит, что можно безопасно делать `install/reset/uninstall`.
